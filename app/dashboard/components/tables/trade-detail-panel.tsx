      ...
      <div className="flex flex-col h-full min-h-0">
        ...
        <div className="px-4 sm:px-6 py-3 border-b border-border/40 shrink-0 flex items-center justify-between bg-muted/5">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <span className="text-base sm:text-lg font-bold tracking-tight truncate">{trade.instrument}</span>
            <Badge variant={isLong ? 'default' : 'destructive'} className="text-[10px] px-1.5 py-0 h-5 uppercase font-bold shrink-0">
              {isLong ? 'Buy' : 'Sell'}
            </Badge>
            <Badge variant={isWin ? 'default' : isLoss ? 'destructive' : 'secondary'} className="text-xs font-mono font-bold shrink-0">
              {formatCurrency(netPnL)}
            </Badge>
            {session && session !== 'Outside Session' && (
              <Badge variant="outline" className="text-[10px] border-primary/20 bg-primary/5 text-primary hidden md:inline-flex">
                {session}
              </Badge>
            )}
            {killzone && (
              <Badge variant="outline" className="text-[10px] border-warning/20 bg-warning/5 text-warning hidden md:inline-flex">
                {killzone}
              </Badge>
            )}
          </div>
          <div className="flex gap-2 items-center shrink-0">
            <Button variant="secondary" onClick={handleEdit} className="h-8 px-3 rounded-lg text-xs font-semibold">
